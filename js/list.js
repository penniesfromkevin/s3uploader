...
function renderRow(item, cols) {
  ...
  row += '<a href="' + item.href + '">' + item.keyText;
  extension = item.href.split('.').pop().toUpperCase();
  if (['PNG', 'GIF', 'JPG', 'JPEG'].indexOf(extension) >= 0) {
    row += '<div align="left"><img src="' + item.href + '" width="500" /><br />' + item.href + '</div>';
  }
  row += '</a>';
  return row;
}
